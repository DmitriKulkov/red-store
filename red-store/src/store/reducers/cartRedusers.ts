...
export const cartReducer = (state = initialCartState, action: SetItemsAction):CartState =>{
    switch (action.type){
        case itemsActionType.SET_ITEMS:
            state = { ...state, items: action.items}
            return state
        case itemsActionType.ADD_ITEM:
            state = { items: [...state.items, {...action.item, id: state.items.length !== 0? state.items[state.items.length - 1].id + 1:0}] }
            return state
